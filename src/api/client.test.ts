...
import { type FetchFn, NiaClient } from "./client";
...
describe("NiaClient", () => {
	...
	it("builds GET query strings and forwards auth headers", async () => {
		...
		expect(requestUrl).toBe(
			"https://apigcp.trynia.ai/v2/sources?limit=10&search=abc&active=true",
		);
	...
	it("serializes POST JSON bodies", async () => {
		...
		const result = await client.post<{ id: string }>("/contexts", {
			title: "Doc",
		});
	...
	it("serializes PATCH JSON bodies", async () => {
		...
		const result = await client.patch<{ updated: boolean }>("/contexts/ctx_1", {
			title: "Renamed",
		});
	...
	describe("resolveTimeout", () => {
		it("uses LONG_TIMEOUT_MS (120s) for /search/universal endpoint", async () => {
			...
			const result = await client.get("/search/universal", { query: "test" });
			expect(result).toEqual({ ok: true });
		}, 90_000);

		it("uses default timeout (30s) for regular /search endpoint", async () => {
			const client = new NiaClient({
				apiKey: "nia-key",
				timeout: 5_000,
				fetchFn: async (_url, init) => {
			...
			const result = await client.get("/search", { query: "test" });
			expect(result).toContain("timeout_error");
			expect(result).toContain("5000ms");
		}, 90_000);

		it("uses LONG_TIMEOUT_MS for oracle endpoints", async () => {
			const client = new NiaClient({
				apiKey: "nia-key",
				timeout: 5_000,
				fetchFn: async (_url, init) => {
			...
			const result = await client.get("/oracle/jobs/job_123");
			expect(result).toEqual({ ok: true });
		}, 90_000);

		it("uses LONG_TIMEOUT_MS for tracer endpoints", async () => {
			const client = new NiaClient({
				apiKey: "nia-key",
				timeout: 5_000,
				fetchFn: async (_url, init) => {
			...
			const result = await client.get("/tracer/trace_123");
			expect(result).toEqual({ ok: true });
		}, 90_000);

		it("allows explicit timeout override", async () => {
			const client = new NiaClient({
				apiKey: "nia-key",
				fetchFn: async (_url, init) => {
					const signal = init?.signal as AbortSignal | undefined;
	...
	describe("stream", () => {
		...
		it("yields SSE events from stream", async () => {
			const client = new NiaClient({
				apiKey: "nia-key",
				fetchFn: createFetchMock([sseResponse("data: hello\ndata: world\n")]),
			});
		...
		it("includes query params in URL", async () => {
			...
			for await (const _ of client.stream("/oracle/jobs", {
				status: "pending",
			})) {
				// consume
			}
		...
		it("yields error event when response body is null", async () => {
			const client = new NiaClient({
				apiKey: "nia-key",
				fetchFn: createFetchMock([() => new Response(null, { status: 200 })]),
			});
		...
		it("supports abort signal", async () => {
			...
			const client = new NiaClient({
				apiKey: "nia-key",
				fetchFn: createFetchMock([sseResponse("data: start\n")]),
			});
