...
const RESOURCE_TYPE_SCHEMA = tool.schema.enum(["repository", "data_source", "research_paper", "category"]);
...
function resourcePath(resourceType: NiaResourceType, resourceId?: string): string {
...
async function requestDeletePermission(
  ...
  try {
    const result = (await (context.ask as unknown as (input: {
      permission: string;
      patterns: string[];
      always: string[];
...
export function createNiaManageResourceTool(client: NiaClient, config: NiaConfig) {
  return tool({
    ...
    args: {
      ...
      resource_id: tool.schema.string().trim().min(1).optional().describe("Resource or category identifier"),
      name: tool.schema.string().trim().min(1).optional().describe("New name or category name"),
    ...
    async execute(args, context) {
      switch (args.action) {
        ...
        case "status": {
          ...
          const response = await client.get(resourcePath(identity.resourceType, identity.resourceId), undefined, context.abort);
          return typeof response === "string" ? response : jsonResult(response);
        }
        ...
        case "delete": {
          ...
          const approved = await requestDeletePermission(context, identity.resourceType, identity.resourceId);
          if (!approved) {
            return "Delete cancelled.";
          }

          const response = await client.delete(resourcePath(identity.resourceType, identity.resourceId), undefined, context.abort);
          return typeof response === "string" ? response : jsonResult(response);
        }
        ...
        case "category_list": {
          const response = await client.get(resourcePath("category"), undefined, context.abort);
          return typeof response === "string" ? response : jsonResult(response);
        }
        ...
        case "category_delete": {
          ...
          const approved = await requestDeletePermission(context, "category", args.resource_id);
          if (!approved) {
            return "Delete cancelled.";
          }

          const response = await client.delete(resourcePath("category", args.resource_id), undefined, context.abort);
          return typeof response === "string" ? response : jsonResult(response);
        }
      }
    },
