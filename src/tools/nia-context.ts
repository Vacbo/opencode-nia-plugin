...
const VALID_ACTIONS: readonly ContextAction[] = [
	"save",
	"list",
	"retrieve",
	"search",
...
const ABORT_ERROR = "abort_error [nia_context]: request aborted";
const formatError = createToolErrorFormatter("context");
...
async function handleSave(
	...
	const result = (await client.post("/contexts", body)) as ContextResponse;
...
async function handleList(
	...
	const result = (await client.get("/contexts", params)) as ContextListResponse;
...
async function handleRetrieve(
	...
	const result = (await client.get(`/contexts/${args.id}`)) as ContextResponse;
...
async function handleDelete(
	...
	await client.delete(`/contexts/${args.id}`);
...
export function createNiaContextTool(client: SdkAdapter, config: NiaConfig) {
	return tool({
		description:
		...
		args: {
			...
			id: tool.schema
				.string()
				.optional()
				.describe("Context ID (required for retrieve, update, delete)"),
			title: tool.schema
				.string()
				.optional()
				.describe("Context title (required for save)"),
			summary: tool.schema
				.string()
				.optional()
				.describe("Brief summary of the context"),
			content: tool.schema
				.string()
				.optional()
				.describe("Full context content (required for save)"),
			tags: tool.schema
				.string()
				.optional()
		...
		async execute(args, context) {
			try {
				...
				if (!config.contextEnabled) {
					return "config_error: nia context is disabled";
				}
			...
			return await handler(client, args as ContextArgs, context);
			} catch (error) {
