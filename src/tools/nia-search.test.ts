...
describe("nia_search tool", () => {
  it("formats universal search results as markdown and forwards defaults", async () => {
          ...
          results: [
            {
              ...
              content: "Use embeddings and chunking for retrieval quality.",
              url: "https://example.com/guide",
              file_path: "docs/search.md",
              score: 0.98,
              highlights: ["embeddings", "chunking"],
            },
          ],
        };
      },
    ...
    const niaSearchTool = createNiaSearchTool({
      client,
      config: { apiKey: "nia-key", searchEnabled: true },
    });
        ...
        sources: [
          {
            ...
            content: "The client wraps fetch with retries and typed helpers.",
  ...
  for (const [label, error] of [
    ...
    it(`returns the ${label} client error string without throwing`, async () => {
      ...
      const result = await niaSearchTool.execute(
        niaSearchArgsSchema.parse({ query: "search failures" }),
        createContext()
      );
  ...
  it("returns a friendly empty-results markdown message", async () => {
    const niaSearchTool = createNiaSearchTool({
      client: {
        post: async () => ({ query: "nothing", total: 0, results: [] }),
      },
      config: { apiKey: "nia-key", searchEnabled: true },
    });
  ...
  it("returns an abort string when the request signal is already aborted", async () => {
    ...
    const result = await niaSearchTool.execute(
      niaSearchArgsSchema.parse({ query: "cancel me" }),
  ...
  it("truncates long markdown output and appends a marker", async () => {
    ...
    const result = await niaSearchTool.execute(
      niaSearchArgsSchema.parse({ query: "very long", max_tokens: 120 }),
      createContext()
    );
