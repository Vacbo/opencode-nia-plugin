...
describe("nia_research tool", () => {
  it("formats quick mode results as markdown", async () => {
    ...
    const client = {
      ...
      get: async () => { throw new Error("should not poll quick mode"); },
		...
		const niaResearchTool = createNiaResearchTool(client as unknown as SdkAdapter, TEST_CONFIG);
  ...
  it("submits oracle jobs and returns immediately (fire-and-forget)", async () => {
    ...
    const client = {
      post: async (path: string, body?: unknown) => { capturedPath = path; capturedBody = body; return { id: "oracle_job_1", status: "pending", query: "map the auth flow", created_at: "2026-03-18T10:00:00Z" }; },
    };
		const niaResearchTool = createNiaResearchTool(client as unknown as SdkAdapter, TEST_CONFIG);
    const result = await niaResearchTool.execute(niaResearchArgsSchema.parse({ query: "map the auth flow", mode: "oracle" }), createContext());
    expect(capturedPath).toBe("/oracle/jobs");
    expect(capturedBody).toEqual({ query: "map the auth flow", output_format: "markdown" });
  ...
  it("checks oracle job status when job_id is provided", async () => {
    ...
    const client = {
      ...
      get: async (path: string) => { capturedPath = path; return { id: "oracle_job_2", status: "completed", query: "explain retries", created_at: "2026-03-18T10:00:00Z", completed_at: "2026-03-18T10:00:30Z", result: "Retry with backoff on 429 and 503 responses." }; },
    };
		const niaResearchTool = createNiaResearchTool(client as unknown as SdkAdapter, TEST_CONFIG);
  ...
  for (const [status, text] of [["401", "bad key"], ["429", "slow down (retry-after=2)"]] as const) {
    it(`returns client error strings as-is: HTTP ${status}: ${text}`, async () => {
		const niaResearchTool = createNiaResearchTool({ post: async () => { throw new Error(`HTTP ${status}: ${text}`); }, get: async () => { throw new Error("should not poll after a create error"); } } as unknown as SdkAdapter, TEST_CONFIG);
      ...
      expect(result).toContain(`research_error: HTTP ${status}: ${text}`);
    });
  }
  ...
  it("returns an abort string when the request signal is already aborted", async () => {
    const controller = new AbortController();
    controller.abort();
		const niaResearchTool = createNiaResearchTool({ post: async () => { throw new Error("should not reach client"); }, get: async () => { throw new Error("should not reach client"); } } as unknown as SdkAdapter, TEST_CONFIG);
