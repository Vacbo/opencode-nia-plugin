...
const niaSearchArgsShape = {
  ...
  search_mode: z.enum(["universal", "query", "web", "deep"]).default("universal"),
  ...
  return tool({
    ...
    async execute(rawArgs, context) {
      try {
        ...
        return truncateMarkdown(formatResponse(args, response), args.max_tokens);
      } catch (error) {
...
function formatResponse(args: NiaSearchArgs, response: SearchResponse): string {
  ...
  const sources = args.include_sources ? getNormalizedResults(response, "sources") : [];
  if (sources.length > 0) {
...
function getNormalizedResults(response: SearchResponse, key: "results" | "sources"): NormalizedResult[] {
  const candidate = (response as unknown as Record<string, unknown>)[key];
  if (!Array.isArray(candidate)) {
    return [];
  }

  return candidate.map(normalizeResult).filter((result): result is NormalizedResult => result !== undefined);
}
...
function normalizeResult(item: unknown): NormalizedResult | undefined {
  ...
  const title = stringOrFallback(record.title, record.file_path, record.url, "Untitled result");
  const excerpt = stringOrFallback(record.content, record.snippet, "No excerpt available.");
...
function formatUnexpectedError(error: unknown, wasAborted: boolean): string {
  ...
  if (error instanceof Error) {
    return `search_error: ${error.message}`;
  }

  return `search_error: ${String(error)}`;
}
