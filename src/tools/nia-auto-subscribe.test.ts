...
const FIXTURE_DRY_RUN_RESPONSE = {
  dependencies: [
    ...
    { name: "typescript", version: "5.3.0", ecosystem: "npm", status: "already_tracked" },
  ],
  total_new: 1,
  total_existing: 1,
};
...
const FIXTURE_SUBSCRIBE_RESPONSE = {
  dependencies: [
    { name: "react", version: "18.2.0", ecosystem: "npm", status: "subscribed" },
  ],
  total_new: 1,
  total_existing: 1,
};
...
describe("nia_auto_subscribe tool", () => {
  describe("dry_run mode (default)", () => {
    it("posts to /dependencies with dry_run=true by default", async () => {
      ...
      expect(body.manifest_content).toBe('{"dependencies":{"react":"^18.2.0"}}');
    ...
    it("does NOT call context.ask() for dry_run=true", async () => {
      ...
      await tool.execute(
        ...
        createMockContext({ ask: async () => { askCalled = true; } }),
      );
