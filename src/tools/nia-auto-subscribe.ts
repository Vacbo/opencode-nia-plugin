...
const ABORT_ERROR = "abort_error [nia_auto_subscribe]: request aborted";
const formatError = createToolErrorFormatter("auto_subscribe");
	...
	return tool({
		...
		args: {
			...
			manifest_type: tool.schema
				.string()
				.describe(
					"Manifest file type (package.json, requirements.txt, Cargo.toml, go.mod)",
				),
			dry_run: tool.schema
				.string()
				.optional()
				.describe(
					"Preview mode (default: 'true'). Set to 'false' to actually subscribe — requires permission.",
				),
		},
		async execute(args, context) {
			try {
				...
				if (!config.searchEnabled) {
					return "config_error: nia auto-subscribe is disabled";
				}
