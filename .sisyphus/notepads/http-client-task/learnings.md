...
- 2026-03-19: `src/hooks/system-transform.ts` can dedupe pre-LLM system nudges by storing injected hint keys in `sessionState.projectContext`, while `pendingOps.checkAndDrain()` provides one-shot async completion notices and package.json-driven tech-stack hints stay easy to cache per workspace root.
- 2026-03-19: `nia_tracer` now follows the same submit+poll contract as `nia_research` oracle mode: deep submits return a `job_id` immediately, and later `job_id` calls do a single status fetch with no internal timers.
